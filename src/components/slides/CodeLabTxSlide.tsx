  ...
  return (
    <div className="flex flex-col h-full w-full p-4 md:p-6 text-white bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 shadow-[0_0_30px_rgba(59,130,246,0.3)]">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-3xl font-bold text-blue-400">Code Lab: ฝั่งผู้ส่ง (TX)</h2>
      </div>

      <div className="flex flex-col md:flex-row gap-6 flex-1 min-h-0">
        {/* Left Column: Code Logic */}
        <div className="w-full md:w-1/2 flex flex-col min-h-0 relative">
          <div className="absolute top-0 right-0 p-2 z-10 text-slate-500 font-mono text-xs">transmitter.ino</div>
        ...
        <div className="w-full md:w-1/2 flex flex-col min-h-0">
          <div className="flex bg-black/40 p-1 rounded-xl mb-4 border border-white/5">
            <button 
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-semibold transition-all ${rightTab === 'output' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:text-gray-200'}`}
            >
              <Terminal size={16} /> Serial Output
            </button>
            <button 
              ...
              {rightTab === 'output' ? (
                ...
                >
                  <div className="flex items-center gap-2 text-gray-500 text-xs mb-4 border-b border-white/10 pb-2">
                    <Terminal size={12} /> COM3 - 9600 baud
                  </div>
                  <div className="space-y-1">
                    <p className="text-gray-400">Initializing NRF24L01...</p>
                    <p className="text-gray-400">Radio is ready.</p>
                    <p className="text-gray-400">Setting TX address: 00001</p>
                    <p>Sending: Hello IoT</p>
                    <p>Sending: Hello IoT</p>
